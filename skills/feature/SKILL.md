...
Create a `feature/<name>` branch from an up-to-date default branch.
...
1. **Check for an existing branch first.** Run `git branch -a | grep -i <name>` to see if a `feature/<name>` branch (or similar) already exists locally or on the remote. If it does, ask the user if they want to switch to it (and rebase onto `<base>`) instead of creating a new one.
